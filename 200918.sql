...
테이블 생성
테이블명 - ranger
컬럼은 ranger_no_NUMBER
      ranger_nm_VARCHAR2(50),
      reg_ddt DATE
...
INSERT INTO ranger (ranger_no, ranger_nm, reg_dt) 
VALUES (1, 'brown', SYSDATE);


ROLLBACK; --이건 잘못됬ㅇ르때 커밋하기전 데이터 빽하는거니 엔간해서 건들지말자
...
3. 날짜 : DATE - 7 byte고정
            일자, 시간(시,분,초) 정보 저장
            varchar2 날짜관리 : YYYY-MMDD ==> '20200918' ==> 8BYTE
...
제약조건 : deptno 컬럼을 PRIMARY KEY 제약 조건으로 생성
...
위 문장대로 하면 오류가 나는데 NULL이 들어갈수 없기 때문이다
...
정상적으로 입려고딜 수 없다.
unique constraint (HR.SYS_C007083) violated
INSERT INTO dept_test VALUES (90, 'ddit', 'daejeon');
...
명명 규칙 : PK_테이블명
deptno NUMBER(2) PRIMARY KEY 제야조건 PRIMARY KEY,
...
UNIQUE 제약조건 : 값의 중복이 없도록 방지하는 제약조건, 단 NULL은 허용된다.
...
CREATE TABLE dept_test (
    ...
    dname VARCHAR2(14),
    loc VARCHAR2(13),
    CONSTRAINT U_dept_test_test_dname UNIQUE (dname)
); 
...
2. 90번 부서에 속한 9000번 사원의 deptno 컬럼의 값이 NULL로 설정됨
DROP TABLE emp_test;
...
위에 문장치고 밑에 문장치면 dept테이블 내용이 복사된다.
SELECT *
FROM dept_test;
