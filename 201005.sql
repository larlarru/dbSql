...
1. 인위적으로 여러개의 행을 만들기
    ...
    SELECT LEVEL, dummy,
            LTRIM(SYS_CONNECT_BY_PATH(dummy, '-'), '-')
    FROM dual
    CONNECT BY LEVEL <= 31;
    ...
    EX : '202010' ==> 31
          ...
          SELECT TO_DATE('202010', 'YYYYMM') + LEVEL-1 day,
                    ...
                    TO_CHAR(TO_DATE('202010', 'YYYYMM') + LEVEL-1, 'D') iw
          FROM dual
          CONNECT BY LEVEL <= TO_CHAR(LAST_DAY(TO_DATE('202010', 'YYYYMM')), 'DD');
          ...
          SELECT  day, d, iw, DECODE(d, 1, day) d1,
                  DECODE(d, 2, day) d2, DECODE(d, 3, day) d3,
                  DECODE(d, 4, day) d4, DECODE(d, 5, day) d5,
          ...
          SELECT  iw, MIN(DECODE(d, 1, day)) d1,
                  MIN(DECODE(d, 2, day)) d2, MIN(DECODE(d, 3, day)) d3,
                  MIN(DECODE(d, 4, day)) d4, MIN(DECODE(d, 5, day)) d5,
...
절차적인 처리가 필요한 부분은 존재한다 : 인사 시스템 급여, 연말 정산
...
BEGIN
    ...
    /*System.out.print("v_deptno : " + v_deptno + ", v_dname : " + v_dname);*/
...
EXEC registdept_test(30, 'SALES', 'CHICAGO');
...
CREATE OR REPLACE PROCEDURE UPDATEdept_test (p_deptno IN dept.deptno%TYPE,
                    ...
                    p_loc IN dept.loc%TYPE) IS
BEGIN
    UPDATE dept_test SET deptno = p_deptno WHERE deptno = 99;
    UPDATE dept_test SET dname = p_dname WHERE dname = 'ddit';
    UPDATE dept_test SET loc = p_loc WHERE loc = 'daejeon';
    COMMIT;
END;
/
...
rollback;
 ...
 UPDATE dept_test SET deptno = 99;
    UPDATE dept_test SET dname = 'ddit';
    UPDATE dept_test SET loc = 'daejeon';
