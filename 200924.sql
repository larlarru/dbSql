...
오라클 파티션 기능 (오라클 공식 버젼에서만 지원, XE에서는 지원 하지 않음)
...
9000, 'brown' 데이터를 emp_test 넣을려고 하는데
...
WHEN NOT MATCHED THEN
    INSERT [(컬럼1, zjffja2...)] VALUES (값1, 값2.....);
...
SELECT DECODE(GROUPING(job), 1, '총',job) job, 
        DECODE(GROUPING(job) + GROUPING(deptno), 1, '소계',
                                 2, '계',
                                deptno) deptno, 
        GROUPING(job), GROUPING(deptno),
        GROUPING(job) || GROUPING(deptno),
...
실습3 --다시해보기
SELECT DECODE(GROUPING(deptno), 1, '30',
                                deptno) deptno, job,
        GROUPING(job), GROUPING(deptno),
        SUM(sal + NVL(comm, 0)) sal
FROM emp
...
실습 GROUP_AD4 --이것도 답이 틀림

SELECT dept.dname, emp.job,
        SUM(emp.sal + NVL(comm, 0)) sal
FROM emp , dept
